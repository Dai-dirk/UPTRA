...
void Graphviz::drawDFG(){
    ...
    /*for(auto& elem : dfg->inputs()){
        ...
        int lat = _mapping->getLatest(elem.first);
        ofs << quoteName << "[label = \"\\N\\nlat=" << lat << "\"];\n";
        std::cout << "mapping1281 a ha input~~~ "<<std::endl;
    }
    for(auto& elem : dfg->outputs()){
        ...
        std::cout << "mapping1281 a ha output~~~ "<<std::endl;
    }*/
    for(auto& elem : dfg->nodes()){
        ...
        std::string quoteName = "\"" + name + "\"";
        ofs << quoteName << "[label = \"\\N\\nlat=" << _mapping->getLatest(node->id()) + node->opLatency() << "\"];\n";
        ...
        //std::cout << "mapping1281 a ha ~~~ "<<std::endl;
    }
    ofs << "}\n";
}
