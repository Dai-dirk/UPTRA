...
private:
    ...
    int _bitWidth;
    int _selfloop = -1 ; // selfloop index, -1: no self loop  
    int _alap = 0;  // ALALP value
    int _asap = 0;  // ALALP value
...
public:
    ...
    //new distance of iteration
    int selfloop(){ return _selfloop; }
    void setSelfloop(int loop){ _selfloop = loop; }
    //new ALAP value
    int getALAP(){ return _alap; }
    void setALAP(int alap){ _alap = alap; }
    //new ASAP value
    int getASAP(){ return _asap; }
    void setASAP(int asap){ _asap = asap; }
    ...
    // set LOAD or STORE operation, latency, commutative according to operation name
    void setLSOperation(std::string operation);
    ...
    virtual void print();
    //new set paprameter for cycle
    void setCritical(){_critical = true;};
    bool isCritical(){return _critical;};
    void setCycleID(int cycleID){ _cycleID = cycleID;};
    int getCycleID(){return _cycleID;};
};
