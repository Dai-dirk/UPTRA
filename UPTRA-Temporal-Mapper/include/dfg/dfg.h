...
private:
    ...
    int _bitWidth;
    std::list<std::list<DFGEdge*>*>* m_cycleLists;
    // int _numInputs;
    // int _numOutputs;   
    ...
    std::map<int, std::set<std::pair<int, int>>> _stores; //new <storenode-index, set<node-id, node-port-idx>>
    ...
    std::map<std::pair<int, int> , int> _distance;   // <<srcId, dstId> , distance>
...
public:
    ...
    int numStores(){ return _stores.size(); }              //new
    ...
    const std::map<int, std::set<std::pair<int, int>>>& stores(){ return _stores; }
    ...
    std::set<std::pair<int, int>> store(int index); // return set<node-id, node-port-idx>
    void addInput(int index, std::pair<int, int> node);  // add input
    void addOutput(int index, std::pair<int, int> node); // add output
    void addStore(int index, std::pair<int, int> node);  // add store
    ...
    void delStore(int index); // delete store  new
    void delStore(int index, std::pair<int, int> node);
    ...
    std::list<std::list<DFGEdge*>*>* getCycles();
    void DFS_cycle(int headId, int currentId, std::list<DFGEdge*>* t_erasedEdges, std::list<DFGEdge*>* t_currentCycle,\
